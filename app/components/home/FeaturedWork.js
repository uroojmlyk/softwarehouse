...
export default function FeaturedWork() {
  ...
  if (!mounted) {
    return (
      <section className="relative overflow-hidden section-padding bg-gradient-to-b from-[#0F0F0F] to-[#1A1A1A]">
        <div className="px-4 container-custom">
          <div className="text-center">
            <h2 className="text-white">Loading projects...</h2>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative overflow-hidden section-padding bg-gradient-to-b from-[#0F0F0F] to-[#1A1A1A]"
    >
      {/* Simple Background */}
      ...
      <div className="relative z-10 px-4 container-custom sm:px-6">
        ...
        >
          <span className="inline-block px-4 py-2 mb-4 text-xs border rounded-full text-accent border-accent/30 bg-accent/5">
            ✦ SUCCESS STORIES ✦
          </span>
        ...
        {/* Success Metrics - Responsive Grid */}
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="visible"
        ...
        >
          {successMetrics.map((metric, index) => (
            ...
            >
              <div className="mb-1 text-xl">{metric.icon}</div>
              <div className="text-lg font-display text-accent">{metric.value}</div>
        ...
        >
          {projects.map((project, index) => (
            <motion.div
              key={project.id}
              variants={itemVariants}
              className="overflow-hidden transition-colors border rounded-lg bg-white/5 border-white/10 hover:border-accent/30"
            >
              <div className="flex flex-col md:flex-row">
                {/* Image */}
                <div className="relative flex-shrink-0 w-full h-48 md:w-48 md:h-auto">
                  <Image
                    src={project.image}
                    alt={project.title}
                    fill
                  ...
                  <span className="absolute px-2 py-1 text-xs border rounded-full top-2 left-2 bg-black/60 text-accent border-accent/30 md:hidden">
                    {project.category}
                  </span>
                </div>
                ...
                <div className="flex-1 p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className={`text-lg text-white ${fontClasses.display}`}>
                        {project.title}
                      </h3>
                      <p className="text-xs text-accent">{project.client}</p>
                    </div>
                    <span className="hidden text-2xl md:block">{project.icon}</span>
                  </div>

                  <p className={`text-xs text-gray-400 mb-3 line-clamp-2 ${fontClasses.body}`}>
                    {project.description}
                  </p>
                  ...
                  <div className="mb-3">
                    <div className="flex flex-wrap gap-2">
                      {project.results.map((result, idx) => (
                        <span key={idx} className="text-[10px] px-2 py-1 bg-accent/10 text-accent rounded">
                          {result}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {project.tags.slice(0, 3).map((tag, idx) => (
                      <span key={idx} className="text-[10px] px-2 py-1 bg-white/10 text-gray-300 rounded">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <Link 
                    href={`/work/${project.id}`}
                    className="inline-flex items-center gap-1 text-xs transition-colors text-accent hover:text-white"
                  >
                    <span>View Case Study</span>
                    <span>→</span>
                  </Link>
        ...
        >
          <Link 
            href="/work"
            className="inline-block px-6 py-3 text-sm transition-colors border rounded-lg text-accent border-accent/30 hover:bg-accent hover:text-primary"
          >
            View All Projects →
          </Link>
        </motion.div>
