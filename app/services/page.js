...
export default function ServicesPage() {
  ...
  // Mouse follow spring animation - only on desktop
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);
  ...
  const handleMouseMove = (e) => {
    if (windowWidth < 1024) return; // Disable on mobile/tablet
    const rect = e.currentTarget.getBoundingClientRect();
    mouseX.set(e.clientX - rect.left);
    mouseY.set(e.clientY - rect.top);
  };
  ...
  const cardVariants = {
    ...
    visible: {
      opacity: 1,
      scale: 1,
      transition: { duration: windowWidth < 768 ? 0.3 : 0.5, ease: "easeOut" }
    },
    hover: {
  ...
  // ✅ OPTIMIZED: Reduce particles on mobile
  const particleCount = windowWidth < 768 ? 10 : 30;
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#0B0B0B] via-[#151515] to-[#1E1E1E] pt-16 md:pt-20 lg:pt-24 xl:pt-32 overflow-x-hidden">
      {/* ✅ OPTIMIZED: Animated Background - reduced on mobile */}
      <div className="fixed inset-0 pointer-events-none">
          ...
          <div 
            ...
            style={{
              background: windowWidth < 768 
                ...
                : `radial-gradient(circle at 20% 50%, rgba(201, 169, 89, 0.15) 0%, transparent 50%),
                   radial-gradient(circle at 80% 50%, rgba(201, 169, 89, 0.1) 0%, transparent 50%)`,
            }}
          />
        </div>
        ...
        {[...Array(particleCount)].map((_, i) => (
          <motion.div
            ...
            animate={{
              y: [0, windowWidth < 768 ? -30 : -50, 0],
              x: [0, (Math.random() * (windowWidth < 768 ? 15 : 25)) - (windowWidth < 768 ? 7.5 : 12.5), 0],
              opacity: [0.1, 0.3, 0.1],
            }}
            transition={{
              duration: windowWidth < 768 ? 5 + Math.random() * 5 : 8 + Math.random() * 8,
              repeat: Infinity,
              delay: Math.random() * 5,
            }}
          />
      ...
      <section className="relative min-h-[40vh] md:min-h-[50vh] lg:min-h-[60vh] flex items-center justify-center section-padding">
        <div className="relative z-10 px-4 container-custom sm:px-6 lg:px-8">
          ...
          >
            ...
            {/* ✅ OPTIMIZED: Interactive Tech Cloud - responsive */}
            <motion.div 
              variants={itemVariants}
              ref={constraintsRef}
              onMouseMove={handleMouseMove}
              className="relative flex items-center justify-center h-24 sm:h-28 md:h-32 lg:h-36 xl:h-40"
            >
              {technologies.slice(0, windowWidth < 768 ? 6 : 10).map((tech, index) => {
                const angle = (index / (windowWidth < 768 ? 6 : 10)) * Math.PI * 2;
                const radius = windowWidth < 768 ? 40 : (windowWidth < 1024 ? 60 : 80);
                const x = Math.cos(angle) * radius;
                const y = Math.sin(angle) * radius;
                ...
                return (
                  <motion.div
                    ...
                    className="absolute"
                    animate={{
                      x: [x, x + (windowWidth < 768 ? 10 : 20), x],
                      y: [y, y + (windowWidth < 768 ? 5 : 10), y],
                    }}
                    transition={{
                      duration: windowWidth < 768 ? 3 + index : 5 + index,
                      repeat: Infinity,
                      ease: "easeInOut"
                    }}
                    whileHover={{ scale: windowWidth < 768 ? 1.05 : 1.1 }}
                  >
                    <div className="px-2 md:px-3 lg:px-4 py-1 md:py-1.5 lg:py-2 text-[8px] md:text-xs lg:text-sm border rounded-full bg-black/40 backdrop-blur-md border-accent/20 text-accent whitespace-nowrap">
                      <span className="mr-1 md:mr-2">{tech.icon}</span>
      ...
      {/* ✅ OPTIMIZED: Services Grid - Responsive */}
      <section className="section-padding">
        <div className="px-4 container-custom sm:px-6 lg:px-8">
          <motion.div
            ...
            variants={containerVariants}
            className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 md:gap-5 lg:gap-6"
          >
            {services.map((service, index) => (
              <motion.div
                key={service.id}
                ...
                whileHover="hover"
                onHoverStart={() => setActiveService(index)}
                className="relative overflow-hidden transition-all duration-500 border rounded-lg group bg-gradient-to-br from-white/5 to-transparent backdrop-blur-sm border-white/10 hover:border-accent/30 md:rounded-xl lg:rounded-2xl"
              >
                {/* Animated Background on Hover */}
                <motion.div
                  className="absolute inset-0 bg-gradient-to-br from-accent/10 to-transparent"
                ...
                {/* Corner Accent */}
                <div className="absolute top-0 right-0 w-16 h-16 md:w-20 lg:w-24 md:h-20 lg:h-24">
                  <div className="absolute top-0 right-0 w-10 h-10 rounded-bl-full md:w-12 lg:w-16 md:h-12 lg:h-16 bg-gradient-to-bl from-accent/20 to-transparent" />
                </div>

                <div className="relative p-4 md:p-5 lg:p-6 xl:p-8">
                  {/* Icon with Glow */}
                  <motion.div
                    className="mb-3 text-3xl md:mb-4 lg:mb-5 xl:mb-6 md:text-4xl lg:text-5xl xl:text-6xl"
                    animate={{
                      scale: activeService === index && windowWidth >= 768 ? [1, 1.2, 1] : 1,
                      rotate: activeService === index && windowWidth >= 768 ? [0, 10, -10, 0] : 0
                    }}
                    transition={{ duration: 0.5 }}
                  >
                    {service.icon}
                  ...
                  {/* Title */}
                  <h3 className={`text-base md:text-lg lg:text-xl xl:text-2xl ${fontClasses.display} text-white mb-1 md:mb-2`}>
                    {service.title}
                  </h3>
                  
                  <p className={`text-accent text-[10px] md:text-xs lg:text-sm mb-2 md:mb-3 ${fontClasses.body}`}>
                    {service.tagline}
                  </p>

                  {/* Description - Truncated */}
                  <p className={`text-gray-400 text-[10px] md:text-xs lg:text-sm mb-3 md:mb-4 line-clamp-2 ${fontClasses.body}`}>
                    {service.description}
                  </p>

                  {/* Stats Row */}
                  <div className="flex gap-2 mb-3 md:gap-3 lg:gap-4 md:mb-4">
                    {Object.entries(service.stats).map(([key, value]) => (
                      <div key={key} className="text-center">
                        <div className={`text-xs md:text-sm lg:text-base font-display text-accent`}>
                          {value}
                        </div>
                        <div className="text-[8px] md:text-[10px] lg:text-xs text-gray-500 capitalize">
                          {key}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-1 mb-3 md:gap-2 md:mb-4">
                    {service.capabilities.slice(0, windowWidth < 768 ? 1 : 2).map((cap, idx) => (
                      <span 
                        key={idx}
                        className="px-1.5 md:px-2 py-0.5 md:py-1 text-[8px] md:text-[10px] lg:text-xs border rounded-full bg-accent/10 border-accent/20 text-accent"
                      >
                        {cap}
                      </span>
                    ))}
                    {service.capabilities.length > (windowWidth < 768 ? 1 : 2) && (
                  ...
                  <Link 
                    href={`/services/${service.title.toLowerCase().replace(/\s+/g, '-')}`}
                    className="inline-flex items-center gap-1 text-[10px] md:text-xs lg:text-sm transition-colors text-white/60 hover:text-accent group/link"
                  >
                    <span>Learn More</span>
                    <motion.span
                      animate={{ x: [0, 3, 0] }}
                ...
                {/* Bottom Gradient Line */}
                <motion.div 
                  className={`absolute bottom-0 left-0 right-0 h-0.5 md:h-1 bg-gradient-to-r ${service.gradient}`}
                  initial={{ scaleX: 0 }}
                  whileInView={{ scaleX: 1 }}
                  viewport={{ once: true }}
      ...
      {/* ✅ OPTIMIZED: Process Section - Responsive */}
      <section className="section-padding">
        <div className="px-4 container-custom sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <h2 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl ${fontClasses.display} text-white mb-2 md:mb-3 lg:mb-4`}>
              Our <span className={`bg-gradient-to-r ${goldGradients.light} bg-clip-text text-transparent`}>Process</span>
            </h2>
            <p className={`text-sm md:text-base lg:text-lg text-gray-300 max-w-2xl mx-auto px-4 ${fontClasses.body}`}>
              A proven methodology that ensures your project's success
            </p>
          </motion.div>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-4 md:gap-4 lg:gap-6">
              {[
                ...
                >
                  <div className="relative inline-block">
                    <div className="flex items-center justify-center w-16 h-16 mx-auto text-lg border-2 rounded-full md:w-20 md:h-20 lg:w-24 lg:h-24 md:text-xl lg:text-2xl bg-gradient-to-br from-accent/20 to-transparent border-accent text-accent font-display">
                      {phase.step}
                    </div>
                  ...
                  <h3 className={`text-base md:text-lg lg:text-xl ${fontClasses.display} text-white mt-3 md:mt-4 mb-1 md:mb-2`}>
                    {phase.title}
                  </h3>
      ...
      {/* ✅ OPTIMIZED: Case Studies Mini Grid - Responsive */}
      <section className="section-padding">
        <div className="px-4 container-custom sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            ...
            viewport={{ once: true }}
            transition={{ duration: windowWidth < 768 ? 0.3 : 0.5 }}
            className="mb-6 text-center md:mb-8 lg:mb-12"
          >
            <h2 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl ${fontClasses.display} text-white mb-2 md:mb-3 lg:mb-4`}>
              Recent <span className={`bg-gradient-to-r ${goldGradients.light} bg-clip-text text-transparent`}>Success</span>
            </h2>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 md:gap-5 lg:gap-6">
            {caseStudies.map((study, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.05 }}
                whileHover={{ y: windowWidth < 768 ? -2 : -3 }}
                className="relative p-4 overflow-hidden border rounded-lg md:p-5 lg:p-6 bg-gradient-to-br from-white/5 to-transparent backdrop-blur-sm border-white/10 md:rounded-xl group"
              >
                <div className="mb-2 text-2xl md:mb-3 md:text-3xl lg:text-4xl">{study.icon}</div>
                <h3 className={`text-sm md:text-base lg:text-lg ${fontClasses.display} text-white mb-1 md:mb-2`}>
                  {study.title}
                </h3>
                <p className={`text-accent text-[10px] md:text-xs lg:text-sm mb-1 md:mb-2 ${fontClasses.body}`}>
                  {study.client}
                </p>
      ...
      {/* ✅ OPTIMIZED: FAQ Section - Responsive */}
      <section className="section-padding">
        <div className="max-w-3xl px-4 container-custom sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          ...
          <div className="space-y-3 md:space-y-4">
            {faqs.map((faq, index) => (
              <motion.div
                ...
                className="group"
              >
                <details className="bg-gradient-to-br from-white/5 to-transparent backdrop-blur-sm border border-white/10 rounded-lg p-3 md:p-4 lg:p-5 [&_summary]:list-none cursor-pointer">
                  <summary className="flex items-center justify-between">
                    <h3 className={`text-xs md:text-sm lg:text-base ${fontClasses.display} text-white pr-4`}>
                      {faq.question}
                    </h3>
                    <motion.span
                      className="flex-shrink-0 text-base md:text-lg lg:text-xl text-accent"
                      whileHover={{ scale: windowWidth < 768 ? 1.1 : 1.2 }}
                    >
                      ✦
                    </motion.span>
                  </summary>
                  <motion.p
                    initial={{ opacity: 0, y: -10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className={`mt-2 md:mt-3 text-[10px] md:text-xs lg:text-sm text-gray-400 ${fontClasses.body}`}
                  >
                    {faq.answer}
                  </motion.p>
                </details>
      ...
      <section className="section-padding">
        <div className="px-4 container-custom sm:px-6 lg:px-8">
          <motion.div
            ...
            className="relative"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-accent/20 to-transparent blur-2xl" />
            
            <div className="relative p-4 overflow-hidden text-center border rounded-lg md:p-6 lg:p-8 xl:p-12 bg-gradient-to-br from-accent/10 to-transparent backdrop-blur-sm border-accent/20 md:rounded-xl lg:rounded-2xl">
              ...
              <div className="relative z-10">
                <motion.h2 
                  ...
                  whileInView={{ scale: 1 }}
                  viewport={{ once: true }}
                  className={`text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl ${fontClasses.display} text-white mb-2 md:mb-3 lg:mb-4 px-2`}
                >
                  Ready to Build Something{' '}
                ...
                <p className={`text-xs md:text-sm lg:text-base text-gray-300 mb-4 md:mb-5 lg:mb-6 max-w-2xl mx-auto px-2 ${fontClasses.body}`}>
                  Let's discuss how our services can transform your business.
                </p>

                <div className="flex flex-col justify-center gap-2 px-2 sm:flex-row md:gap-3 lg:gap-4">
                  <Link 
                    href="/contact"
